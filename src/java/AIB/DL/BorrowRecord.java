...
public class BorrowRecord implements Serializable {
    ...
    public List<BorrowBean> getBorrowRecords(long shopId, String filterState, String sortBy) throws SQLException {
        ...
        String sql = "SELECT b.id AS borrowId, b.DT AS borrowDate, s1.name AS fromShop, s2.name AS toShop, " +
    ...
    private boolean updateFruitStock(Connection conn, long shopId, long fruitId, int quantityChange) throws SQLException {
        ...
        // 更新庫存數量
        String updateSql = "UPDATE shopStock SET num = num + ? WHERE shopid = ? AND fruitid = ?";
        try (PreparedStatement updateStmt = conn.prepareStatement(updateSql)) {
            updateStmt.setInt(1, quantityChange);
            updateStmt.setLong(2, shopId);
            updateStmt.setLong(3, fruitId);
